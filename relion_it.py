...
class RelionItGui(object):
    ...
    def __init__(self, main_window, options):
        ...
        import_button = new_browse_button(project_frame, self.import_images_var,
                                          filetypes=(
                                          ('Image file', '{*.mrc, *.mrcs, *.tif, *.tiff}'), ('All files', '*')))
    ...
    for line in open(filename):
        ...
        if line.startswith("data_"):
            ...
            assert len(elems) == len(current_colnames), (
                "Error in STAR file {}, number of elements in {} does not match number of column names {}"
...
def RunJobs(jobs, repeat, wait, schedulename):
    ...
    command = 'relion_pipeliner --schedule ' + schedulename + ' --repeat ' + str(repeat) + ' --min_wait ' + str(
        wait) + ' --RunJobs "' + runjobsstring + '" &'
...
def run_pipeline(opts):
    ...
    if opts.do_second_pass:
        ...
        if not secondpass_ref3d == '':
            print(' RELION_IT: found', secondpass_ref3d, 'with angpix=', secondpass_ref3d_angpix,
                  'as a 3D reference for second pass in file', SECONDPASS_REF3D_FILE)
    ...
    for ipass in range(first_pass, nr_passes):
        ...
        if opts.images_are_movies:
            ...
            motioncorr_job, already_had_it = addJob('MotionCorr', 'motioncorr_job', SETUP_CHECK_FILE,
                                                    motioncorr_options)
        ...
        if opts.images_are_movies:
            ctffind_options.append(
                'Input micrographs STAR file: == {}{}'.format(motioncorr_job, 'corrected_micrographs.star'))
        else:
        ...
        print(' RELION_IT: submitted', preprocess_schedule_name, 'pipeliner with', opts.preprocess_repeat_times,
              'repeats of the preprocessing jobs')
        print(
            ' RELION_IT: this pipeliner will run in the background of your shell. You can stop it by deleting the file RUNNING_PIPELINER_' + preprocess_schedule_name)
        ...
        if (ipass == 0 and (opts.do_class2d or opts.do_class3d)) or (
            ...
            if ipass == 1 and (opts.extract_downscale or opts.extract2_downscale):
                ...
                if abs(float(particles_angpix) - float(opts.autopick_ref_angpix)) > 0.01:
                    ...
                    print(' RELION_IT: rescaling the 3D reference from pixel size', opts.autopick_ref_angpix, 'to',
                          particles_angpix, 'and saving the new reference as', opts.class3d_reference)
                    command = 'relion_image_handler --i ' + opts.autopick_3dreference + ' --o ' + opts.class3d_reference + ' --angpix ' + str(
                        opts.autopick_ref_angpix) + ' --rescale_angpix ' + str(particles_angpix) + ' --new_box ' + str(
                        particles_boxsize)
                    os.system(command)
                ...
                for ibatch in range(0, nr_batches):
                    ...
                    if (rerun_batch1 or batch_size == opts.batch_size):
                        ...
                        if opts.do_discard_on_image_statistics:
                            ...
                            discard_job, already_had_it = addJob('Select', discard_job_name, SETUP_CHECK_FILE,
                                                                 discard_options)

                            if ((not already_had_it) or rerun_batch1):
                                have_new_batch = True
                                RunJobs([discard_job], 1, 1, 'DISCARD')
                                print(" RELION_IT: submitted job to discard based on image statistics for", batch_size,
                                      "particles in", batch_name)
                        ...
                        if (ipass == 0 and opts.do_class2d) or (ipass == 1 and opts.do_class2d_pass2):
                            ...
                            class2d_options = ['Input images STAR file: == {}'.format(particles_star_file),
                                               ...
                                               'Angular search range - psi (deg): == {}'.format(
                                                   opts.class2d_angle_step),
                                               ...
                                               'Copy particles to scratch directory: == {}'.format(
                                                   opts.refine_scratch_disk),
                            ...
                            if ((not already_had_it) or rerun_batch1):
                                ...
                                print(" RELION_IT: submitted 2D classification with", batch_size, "particles in",
                                      class2d_job)
                    ...
                    if (ipass == 0 and opts.do_class3d) or (ipass == 1 and opts.do_class3d_pass2):
                        ...
                        if (
                        not opts.have_3d_reference) and ipass == 0 and iibatch == 1 and batch_size == opts.batch_size:
                            ...
                            inimodel_options = ['Input images STAR file: == {}'.format(particles_star_file),
                                                ...
                                                'Number of initial iterations: == {}'.format(
                                                    opts.inimodel_nr_iter_initial),
                                                'Number of in-between iterations: == {}'.format(
                            ...
                            inimodel_job, already_had_it = addJob('InitialModel', 'inimodel', SETUP_CHECK_FILE,
                                                                  inimodel_options)

                            if (not already_had_it):
                                have_new_batch = True
                                RunJobs([inimodel_job], 1, 1, 'INIMODEL')
                                print(" RELION_IT: submitted initial model generation with", batch_size, "particles in",
                                      inimodel_job)
                            ...
                            if sgd_model_star is None:
                                print(
                                    " RELION_IT: Initial model generation " + inimodel_job + " does not contain expected output maps.")
                                print(
                            ...
                            best_inimodel_class, best_inimodel_resol, best_inimodel_angpix = findBestClass(
                                sgd_model_star, use_resol=True)
                        ...
                        if opts.have_3d_reference:
                            ...
                            if ((not already_had_it) or rerun_batch1):
                                ...
                                print(' RELION_IT: submitted 3D classification with', batch_size, 'particles in',
                                      class3d_job)
                            ...
                            if class3d_model_star is None:
                                print(
                                    " RELION_IT: 3D Classification " + class3d_job + " does not contain expected output maps.")
                                print(
                                    " RELION_IT: This job should have finished, but you may continue it from the GUI.")
                                raise Exception("ERROR!! quitting the pipeline.")  # TODO: MAKE MORE ROBUST

                            best_class3d_class, best_class3d_resol, best_class3d_angpix = findBestClass(
                                class3d_model_star, use_resol=True)

                            # Once the first batch in the first pass is completed: move on to the second pass
                            if (
                                    ipass == 0 and opts.do_second_pass and iibatch == 1 and best_class3d_resol < opts.minimum_resolution_3dref_2ndpass):
                                ...
                                if os.path.isfile(filename_to_remove):
                                    print(' RELION_IT: removing file', filename_to_remove,
                                          'to stop the pipeliner from the first pass')
                                ...
                                print(' RELION_IT: moving on to the second pass using', opts.autopick_3dreference,
                                      'for template-based autopicking')
...
def main():
    ...
    print(' RELION_IT: ')
    print(
        ' RELION_IT: this script will check whether processes are still running using files with names starting with RUNNING')
    ...
    print(
        ' RELION_IT:   upon a restart, jobs present in this file will be continued (for preprocessing), or ignored when already finished')
    print(
        ' RELION_IT: if you would like to re-do a specific job from scratch (e.g. because you changed its parameters)')
    ...
    for checkfile in (
    'RUNNING_PIPELINER_' + PREPROCESS_SCHEDULE_PASS1, 'RUNNING_PIPELINER_' + PREPROCESS_SCHEDULE_PASS2):
        if os.path.isfile(checkfile):
            print(" RELION_IT: ERROR:", checkfile,
